...
def main():
    ...
    host.set_facecolor(AXES_COLOR)
    ax1 = host.twinx()  # temp
    ax2 = host.twinx()  # motion
    ax2.spines["right"].set_position(("outward", 50))  # Offset third axis
    ...
    # Set Labels & Ranges
    host.set_xlabel("Time", color=FACE_COLOR, fontweight="bold")
    host.set(ylabel="Light Intensity (Lx)", ylim=(light_min, light_max))
    ax1.set(ylabel="Temperature (°C)", ylim=(temp_min, temp_max))
    ax2.set(ylabel="Motion (Boolean)", ylim=(0, 1))

    host.legend(loc="best")
    host.tick_params(axis="x", colors=FACE_COLOR, labelsize=14, rotation=0)
