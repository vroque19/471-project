...
"""
    # print("sleep day and wake days: ", sleep_day, bed_time, today, wake_time)
    conn = get_db_connection()
...
def get_last_sleep_score():
    today = datetime.today().replace(hour=0, minute=0, second=0, microsecond=0)  # Move to the next day to get the last score
    weekday = today.weekday()
    print(weekday)
    start_of_week = today - timedelta(days=(weekday + 1) % 7)
    print(start_of_week)
    # the last sleep score entry
    query = """
SELECT *
FROM sleep_scores
...
"""
    conn = get_db_connection()
    score = conn.execute(query).fetchall()
    if not score:
        return None
    row = dict(score[0])
    conn.close()
    return row["score"], row["date"], row["day"]
